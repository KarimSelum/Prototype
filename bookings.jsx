...
function BookingsPage({ scenario, onToast }){
  ...
  const [bookings, setBookings] = useStateB(()=>bookingsFor(scenario.id));
  const payouts = payoutsFor(scenario.id);
  const [clients, setClients] = useStateB(()=>clientsFor(scenario.id));
  const [services, setServices] = useStateB(defaultServices());
  const [serviceModal, setServiceModal] = useStateB(false);

  useEffectB(()=>{
    setBookings(bookingsFor(scenario.id));
    setClients(clientsFor(scenario.id));
  },[scenario.id]);

  const addBooking = ({name, email, service, time}) => {
    const price = service.p === 'Free' ? 0 : Number(service.p.replace(/[^\d]/g,'')) || 0;
    const who = name || 'New client';
    const booking = {
      who,
      service: service.n + (service.d ? ' · '+service.d.split(' · ')[0] : ''),
      when: time.replace(/^([A-Za-z]+)\s/, '$1 · '),
      price,
      status: price ? 'paid' : 'free',
      new:true,
      avatar:'linear-gradient(140deg,#F3DFCF,#B8825A)',
      note: email ? `Booked from customer flow · ${email}` : 'Booked from customer flow'
    };
    setBookings(list=>[booking, ...list]);
    setClients(list=>{
      const existing = list.find(c=>c.name.toLowerCase()===who.toLowerCase());
      if(existing) return list.map(c=>c===existing ? {...c, sessions:c.sessions+1, spend:c.spend+price, last:'Today', status:'Active', history:[...(c.history||[]), `${service.n} · Today`]} : c);
      return [{name:who, sessions:1, spend:price, last:'Today', status:'Active', avatar:booking.avatar, history:[`${service.n} · ${time}`], note:'New booking from the customer flow. Atlas will watch rebooking signals after the session.'}, ...list];
    });
    setView('today');
  };
    ...
    <div className="os-page" style={{padding:'32px 40px 60px', maxWidth:1240, margin:'0 auto'}}>
      <div style={{display:'flex', alignItems:'baseline', justifyContent:'space-between', marginBottom:24}}>
        ...
        <div style={{display:'flex', gap:8}}>
          <Pill tone="ghost" size="md" onClick={()=>{navigator.clipboard && navigator.clipboard.writeText('https://stillwater.hatchers.site/book'); onToast({message:'Booking link copied', sub:'stillwater.hatchers.site/book'});}}>Copy booking link</Pill>
          <Pill tone="solid" size="md" onClick={()=>setServiceModal(true)}><Icon.Plus size={12}/> New service</Pill>
        </div>
      </div>
      ...
      {view==='week' && <WeekView bookings={bookings} setBookings={setBookings} scenario={scenario} services={services} onToast={onToast}/>}
      {view==='customer' && <CustomerView services={services} onToast={onToast} onBooked={addBooking}/>}
      ...
      {serviceModal && <ServiceModal onClose={()=>setServiceModal(false)} onSave={(service)=>{setServices(list=>[service,...list]); setServiceModal(false); setView('customer'); onToast({message:'Service added', sub:service.n+' is live on the booking page'});}}/>}
    </div>
  );
}
...
function ServiceModal({onClose,onSave}){
  const [name,setName] = useStateB('Somatic 90min');
  const [duration,setDuration] = useStateB('90min · Lisbon studio');
  const [price,setPrice] = useStateB('€190');
  return (
    <div className="overlay-backdrop" onClick={onClose}>
      <div className="overlay-card" onClick={e=>e.stopPropagation()} style={{width:'min(440px,100%)', padding:22}}>
        <div style={{display:'flex', justifyContent:'space-between', marginBottom:16}}>
          <div><SectionLabel style={{marginBottom:6}}>New service</SectionLabel><div className="serif" style={{fontSize:28}}>Add booking type</div></div>
          <button onClick={onClose} style={{fontSize:22, color:'var(--muted)'}}>×</button>
        </div>
        <label style={{fontSize:11, color:'var(--muted)'}}>Service name</label>
        <input value={name} onChange={e=>setName(e.target.value)} style={{width:'100%', padding:'11px 12px', border:'1px solid var(--line)', borderRadius:8, background:'#fff', margin:'6px 0 12px'}}/>
        <label style={{fontSize:11, color:'var(--muted)'}}>Details</label>
        <input value={duration} onChange={e=>setDuration(e.target.value)} style={{width:'100%', padding:'11px 12px', border:'1px solid var(--line)', borderRadius:8, background:'#fff', margin:'6px 0 12px'}}/>
        <label style={{fontSize:11, color:'var(--muted)'}}>Price</label>
        <input value={price} onChange={e=>setPrice(e.target.value)} style={{width:'100%', padding:'11px 12px', border:'1px solid var(--line)', borderRadius:8, background:'#fff', margin:'6px 0 16px'}}/>
        <Pill tone="solid" size="md" onClick={()=>onSave({n:name || 'New service', d:duration, p:price || 'Free'})}>Publish service</Pill>
      </div>
    </div>
  );
}
...
function TodayView({bookings, scenario, onToast}){
  const [enabled,setEnabled] = useStateB({});
        ...
        <Card>
          ...
          {[
            ...
            <div key={i} style={{padding:'12px 0', borderBottom: i<2?'1px solid var(--line-soft)':'none', display:'flex', gap:10}}>
              ...
              <button onClick={()=>{setEnabled(e=>({...e,[s.t]:true})); onToast({message:'Enabled', sub:s.t});}} style={{fontSize:11, color:enabled[s.t]?'var(--green)':'var(--ink)', textDecoration:'underline', textDecorationColor:enabled[s.t]?'var(--green)':'var(--taupe)', textUnderlineOffset:3}}>{enabled[s.t]?'Enabled':'Enable'}</button>
            </div>
          ))}
        </Card>
      </div>
...
function WeekView({bookings, setBookings, scenario, services, onToast}){
  ...
  const [slotService, setSlotService] = useStateB(services[0]?.n || '1:1 Somatic');
                ...
                <div key={di+slot} onClick={()=>!b && !isBlocked && setPopover({key, day:d, slot})} style={{borderLeft:'1px solid var(--line-soft)', borderTop:'1px solid var(--line-soft)', minHeight:42, padding:4, position:'relative', background:b?'var(--bg-2)':isBlocked?'#00000008':'transparent', cursor:b?'default':'pointer'}}>
                  ...
                  {popover && popover.key===key && (
                    <div onClick={e=>e.stopPropagation()} className="fade-up" style={{position:'absolute', zIndex:6, top:36, left:4, width:210, padding:12, border:'1px solid var(--line)', borderRadius:8, background:'#fff', boxShadow:'0 16px 40px -20px #00000070'}}>
                      ...
                      <select value={slotService} onChange={e=>setSlotService(e.target.value)} style={{width:'100%', border:'1px solid var(--line)', borderRadius:7, padding:'8px', fontSize:12, marginBottom:8, background:'#fff'}}>
                      ...
                      <div style={{display:'flex', gap:6, marginBottom:10}}>
                        ...
                        <Pill tone="ghost" size="sm" onClick={()=>{setBookings(list=>[{who:'Open slot', service:slotService+' · available', when:`${popover.day} · ${popover.slot}`, price:0, status:'open', avatar:'linear-gradient(140deg,#E5EDD9,#7A9456)', note:'Available on booking page'}, ...list]); setPopover(null); onToast({message:'Slot opened for bookings', sub:`${popover.day} · ${popover.slot}`});}}>Open</Pill>
                      </div>
                    </div>
                  )}
                </div>
...
function CustomerView({services, onToast, onBooked}){
        ...
        <div style={{padding:'24px 32px 32px'}}>
          ...
          {step===2 && (
            <>
              ...
              <input value={name} onChange={e=>setName(e.target.value)} placeholder="Your name" style={{width:'100%', padding:'12px 14px', borderRadius:8, border:'1px solid var(--line)', fontSize:13, marginBottom:8, background:'#fff'}}/>
              <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="Email" style={{width:'100%', padding:'12px 14px', borderRadius:8, border:'1px solid var(--line)', fontSize:13, marginBottom:8, background:'#fff'}}/>
              <input value={note} onChange={e=>setNote(e.target.value)} placeholder="What brought you here? (optional)" style={{width:'100%', padding:'12px 14px', borderRadius:8, border:'1px solid var(--line)', fontSize:13, marginBottom:14, background:'#fff'}}/>
              {service.p!=='Free' && (
              ...
              <Pill tone="solid" size="lg" onClick={()=>{setStep(3); onBooked && onBooked({name,email,note,service,time}); onToast({message:'New booking · Maya was notified', sub:`${service.n} · ${time}`});}} style={{width:'100%', justifyContent:'center'}}>
          ...
          {step===3 && (
            <div className="fade-up" style={{textAlign:'center', padding:'20px 0'}}>
              ...
              <Pill tone="ghost" size="md" onClick={()=>{setStep(0); setService(null); setTime(null); setName(''); setEmail(''); setNote('');}}>Book another</Pill>
            </div>
          )}
        </div>
      </Card>
