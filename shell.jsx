    ...
    <div className="os-page" style={{padding:'32px 40px 60px', maxWidth:1060, margin:'0 auto'}}>
      ...
      <div className="bookings-grid" style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16}}>
        <Card>
          ...
          <textarea defaultValue={"I work with people who think they've already tried the body stuff. 1:1 sessions and small retreats."} style={{width:'100%', minHeight:92, border:'1px solid var(--line)', borderRadius:8, padding:12, background:'var(--bg-2)', lineHeight:1.5, marginBottom:12}}/>
          <Pill tone="solid" size="sm" onClick={()=>onToast({message:'Profile saved'})}>Save profile</Pill>
        </Card>

        <Card>
          <SectionLabel style={{marginBottom:14}}>Plan</SectionLabel>
          <div className="serif" style={{fontSize:30, marginBottom:4}}>Hatchers OS · €49/month</div>
          ...
          <Pill tone="ghost" size="sm" style={{marginTop:14}} onClick={()=>onToast({message:'Redirecting to Stripe portal…'})}>Manage billing</Pill>
        </Card>
      </div>
        ...
        <div className="stats-grid" style={{display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12}}>
          {[
            ...
            <div key={x.n} style={{padding:14, border:'1px solid var(--line)', borderRadius:8, background:'var(--bg-2)'}}>
              ...
              {!x.live && <button onClick={()=>onToast({message:'Coming soon'})} style={{fontSize:11, color:'var(--ink)', marginTop:10, textDecoration:'underline', textDecorationColor:'var(--taupe)', textUnderlineOffset:3}}>Connect</button>}
            </div>
          ))}
        </div>
      </Card>
      ...
      <Card>
        ...
        <div style={{display:'flex', gap:10}}>
          <input placeholder="Teach Atlas something new…" style={{flex:1, border:'1px solid var(--line)', borderRadius:999, padding:'10px 14px', background:'var(--bg-2)'}}/>
          <Pill tone="solid" size="md" onClick={()=>onToast({message:'Atlas updated · will apply to future content'})}>Save</Pill>
        </div>
      </Card>
    </div>
  );
